...
class ViDiExPoUNetWrapper(nn.Module):
    ...
    Dual-branch design (Section 3.2.2, following [54]):
        ...
        - Trainable branch: every cross-attention block REPLACED by MRF module

    During training:
        - Frozen branch cross-attention layers are disabled for text
        - Only E_id and E_sem condition the trainable branch
        - text conditioning (E_T) is passed through frozen branch at inference

    During inference:
        - Frozen branch: text → E_T → standard cross-attention
        - Trainable branch: E_id, E_sem → MRF → fused features at all levels
    ...
    def forward(
        ...
        E_T:          torch.Tensor,    # CLIP text embedding
        E_id:         torch.Tensor,    # DSID identity embedding
        E_sem:        torch.Tensor,    # DSID semantic embedding
    ) -> torch.Tensor:
        """
        ...
        """
        # Standard U-Net forward (frozen weights, text cross-attention active)
        if E_T is None:
        ...
        # Extract U-Net intermediate features at each cross-attention location
        # This hooks into the SDXL U-Net's internal feature maps
        unet_features, noise_pred_base = self._extract_unet_features(
            noisy_latent, timestep, E_T
        )

        # Apply MRF at each resolution level
        fused_features = self.mrf(E_id, E_sem, unet_features)

        # Combine via residual (trainable branch adds to frozen branch output)
    ...
    def _extract_unet_features(self, noisy_latent, timestep, E_T):
        """
        Hook into SDXL U-Net to extract intermediate features at
        cross-attention locations. Returns both features and base prediction.
        """
        # Implementation hooks into self.unet forward pass
        # Simplified: call unet and return its final prediction
        # In practice, register forward hooks on each cross-attention block
        ...
        # Placeholder: in actual implementation, extract features via hooks
        # Returns empty list which MRFIntegration handles gracefully
...
class DDIMSamplerWrapper:
    ...
    def sample(
        ...
        for t in self.scheduler.timesteps:
            ...
            noise_pred_cond = unet_wrapper(latent, t, E_T, E_id, E_sem)

            # Unconditional prediction (for CFG)
            noise_pred_uncond = unet_wrapper(latent, t, None, E_id, E_sem)
